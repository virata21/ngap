...
type PDUSessionResourceSetupRequestTransfer struct {
	...
	//gNB에게 보내는 SegmentIdList
	//Common.go에 ProtocolIEID_SegmentIdList = 513 정의됨
	//Multi QoS Flow 지원
	//SegmentIdListSequence			  []SegmentIdList					 `optional,reject`

	//Single QoS Flow 지원. 기존 HIN CP지원
	SegmentIdList *SegmentIdList `optional,reject`
}
...
func (msg *PDUSessionResourceSetupRequestTransfer) toIes() (ies []NgapMessageIE, err error) {
	...
	//srv6: inserted by shchoi
	//SegmentIdList encoding을 준비하는 코드
	//SegmentIdListSequence			  []SegmentIdList 인 경우이다.
	/*
		if len(msg.SegmentIdListSequence) > 0 {
			tmp_SegmentIdList := Sequence[*SegmentIdList]{
				c:   aper.Constraint{Lb: 1, Ub: 4096},
				ext: false,
			}
			for i := range msg.SegmentIdListSequence {
				tmp_SegmentIdList.Value = append(tmp_SegmentIdList.Value, &msg.SegmentIdListSequence[i])
			}
			ies = append(ies, NgapMessageIE{
				Id:          ProtocolIEID{Value: ProtocolIEID_SemengtIdList},
				Criticality: Criticality{Value: Criticality_PresentReject},
				Value:       &tmp_SegmentIdList,
			})
		} else {
			err = utils.WrapError("SegmentIdList is nil", err)
			return
		}
	*/
	//SegmentIdListSequence			  *SegmentIdList 인 경우이다.
	if msg.SegmentIdList != nil {
		ies = append(ies, NgapMessageIE{
			Id:          ProtocolIEID{Value: ProtocolIEID_SegmentIdList},
			Criticality: Criticality{Value: Criticality_PresentReject},
			Value:       msg.SegmentIdList,
		})
	} else {
		err = utils.WrapError("SegmentIdList is nil", err)
		return
...
func (decoder *PDUSessionResourceSetupRequestTransferDecoder) decodeIE(r *aper.AperReader) (msgIe *NgapMessageIE, err error) {
	...
	//srv6: inserted by shchoi
	//SegmentIdList []SegmentIdListSequence support
	/*
		case ProtocolIEID_SemengtIdList:
			tmp := Sequence[*SegmentIdList]{
				c:   aper.Constraint{Lb: 1, Ub: 4096},
				ext: false,
			}
			fn := func() *SegmentIdList { return new(SegmentIdList) }
			if err = tmp.Decode(ieR, fn); err != nil {
				err = utils.WrapError("Read SegmentIdList", err)
				return
			}
			msg.SegmentIdListSequence = []SegmentIdList{}
			for _, i := range tmp.Value {
				msg.SegmentIdListSequence = append(msg.SegmentIdListSequence, *i)
			}
	*/

	//SegmentIdList *SegmentIdList support
	case ProtocolIEID_SegmentIdList:
		var tmp SegmentIdList
		if err = tmp.Decode(ieR); err != nil {
			err = utils.WrapError("Read SegmentIdList", err)
			return
		}
		msg.SegmentIdList = &tmp
